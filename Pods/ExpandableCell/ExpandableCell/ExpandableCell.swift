...
open class ExpandableCell: UITableViewCell {
    ...
    open override func layoutSubviews() {
        ...
        arrowImageView.frame = CGRect(x: width - 18, y: (height - 11)/2, width: 8, height: 14)
    }

    func open() {
        self.isOpen = true
        UIView.animate(withDuration: 0.3) {
            self.arrowImageView.layer.transform = CATransform3DMakeRotation(CGFloat(Double.pi), 1.0, 1.0, 0.0)
        }
    }
